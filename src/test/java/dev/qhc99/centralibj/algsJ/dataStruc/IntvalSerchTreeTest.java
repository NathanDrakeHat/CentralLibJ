...
import dev.qhc99.centralibj.algsJ.misc.OrthoLineIntersectTest;
...
class IntvalSerchTreeTest{
  ...
  List<OrthoLineIntersectTest.Point> intervals = new ArrayList<>();
  OrthoLineIntersectTest.Point sect;
  Set<OrthoLineIntersectTest.Point> answers = new HashSet<>();
  int len = 64;

  {
    ...
    for(var s : shuffle){
      intervals.add(new OrthoLineIntersectTest.Point(s, s + rand.nextInt(len / 2 - 1) + 1));
    }
  ...
  void intersectsTest(){
    ...
    for(var r : res){
      var p = new OrthoLineIntersectTest.Point(r.first(), r.second());
