...
class DepthFirstSearchTest {
    ...
    static Graph<DepthFirstSearch.Vertex<String>> makeGraph(){
        var vs = new ArrayList<DepthFirstSearch.Vertex<String>>(6);
        
        for(int i = 0; i < 6; i++){
            vs.add(i, new DepthFirstSearch.Vertex<>(String.valueOf(names.charAt(i))));
        }
        var G = new Graph<>(vs);
    ...
    static Graph<DepthFirstSearch.Vertex<String>> makeTopographicalDemo(){
//            DFS.Vertex[] A = new DFS.Vertex[9];
            var A = new ArrayList<DepthFirstSearch.Vertex<String>>(9);
            String t = "undershorts,pants,belt,shirt,tie,jacket,socks,shoes,watch";
            var names = t.split(",");
            for(int i = 0; i < 9; i++) { A.add(i, new DepthFirstSearch.Vertex<>(names[i])); }
            Graph<DepthFirstSearch.Vertex<String>> G = new Graph<>(A);
    ...
    static Graph<DepthFirstSearch.Vertex<String>> makeStronglyConnectedComponentsDemo(){
        String t = "a,b,c,d,e,f,g,h";
        var names = t.split(",");
        var A = new ArrayList<DepthFirstSearch.Vertex<String>>(names.length);
        for(int i = 0; i < names.length; i++) { A.add(i,new DepthFirstSearch.Vertex<>(names[i])); }
        Graph<DepthFirstSearch.Vertex<String>> G = new Graph<>(A);
        G.addOneNeighbor(A.get(0), A.get(1));
    ...
    void depthFirstSearchTest() {
        var G = makeGraph();
        DepthFirstSearch.depthFirstSearch(G);
        int idx = 0;
    ...
    void topologicalSortTest(){
        ...
        var l = DepthFirstSearch.topologicalSort(graph);
        boolean flag = true;
    ...
    boolean recursiveTopologicalTest(DepthFirstSearch.Vertex<String> target, DepthFirstSearch.Vertex<String> current, Graph<DepthFirstSearch.Vertex<String>> G){
    ...
    void recursiveTopologicalTest(){
        var graph = makeTopographicalDemo();
        boolean flag = true;
        List<DepthFirstSearch.Vertex<String>> t = new ArrayList<>(graph.getAllVertices());
    ...
    void stronglyConnectedComponentsTest(){
        ...
        DepthFirstSearch.stronglyConnectedComponents(G);
        var vertices = G.getAllVertices();
        List<DepthFirstSearch.Vertex<String>> vs = new ArrayList<>(vertices);
    ...
    DepthFirstSearch.Vertex<String> getRoot(DepthFirstSearch.Vertex<String> v){
        var t = v;
        while(t.parent != null){ t = t.parent; }
        return t;
    }
