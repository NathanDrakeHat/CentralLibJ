...
public class CutRod {
    ...
    public static void test(){
        for (int i = 1; i <= 15; i++) {
            System.out.print(i);
            System.out.print(": ");
            CutResult res = CutRod.recursiveCutRod(CutRod.p, i);
    ...
    public static CutResult recursiveCutRod(int[] p, int rod_len){
        ...
        int sum  = res.getSum();
        if(sum == 0){ res.insert(rod_len); }
        else{ res.insert(rod_len - sum); }
        res.setPrice(q);
        return res;
    }
    ...
    private static CutResult topDownCutRod(int[] p, int rod_len, CutResult[] memory){
        if(memory[rod_len - 1] != null){ return memory[rod_len - 1]; }
    ...
    public static class CutResult{
        ...
        public int getPrice(){ return this.max_price; }

        public void setPrice(int t){ this.max_price = t; }
